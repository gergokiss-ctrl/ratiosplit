...
async function main() {
  ...
  if (!gergo) {
    gergo = await prisma.person.create({ data: { name: "Gergő", displayOrder: 1 } });
  } else if (gergo.name !== "Gergő") {
    gergo = await prisma.person.update({ where: { id: gergo.id }, data: { name: "Gergő" } });
  }

  let judit = await prisma.person.findFirst({ where: { displayOrder: 2 } });
  if (!judit) {
    judit = await prisma.person.create({ data: { name: "Judit", displayOrder: 2 } });
  } else if (judit.name !== "Judit") {
    judit = await prisma.person.update({ where: { id: judit.id }, data: { name: "Judit" } });
  }
  ...
  for (let i = 0; i < categories.length; i++) {
    ...
    if (existingBySortOrder) {
      await prisma.category.update({
        where: { id: existingBySortOrder.id },
        data: { name, icon, color, isActive: true },
      });
    } else {
      await prisma.category.upsert({
        where: { name },
        update: { icon, color, sortOrder, isActive: true },
        create: { name, icon, color, sortOrder, isActive: true },
      });
    }
  }
