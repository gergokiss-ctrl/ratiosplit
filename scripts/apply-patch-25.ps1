...
$cssMarker = "/* Patch 25 - Settings and maintenance modal */"
$css = Get-Content $cssPath -Raw -Encoding UTF8
if ($css -notmatch [regex]::Escape($cssMarker)) {
  Add-Content -Path $cssPath -Value @'


/* Patch 25 - Settings and maintenance modal */
.settings-grid {
  display: grid;
...
'@ -Encoding UTF8
  Write-Host "Patch 25 CSS appended."
} else {
  Write-Host "Patch 25 CSS already present."
}

$pagePath = "app/page.tsx"
$content = Get-Content $pagePath -Raw -Encoding UTF8

# Add settings state next to review state.
if ($content -notmatch 'showSettings') {
  $content = $content.Replace('[showReview,setShowReview]=useState(false),[editingExpense,setEditingExpense]', '[showReview,setShowReview]=useState(false),[showSettings,setShowSettings]=useState(false),[backupHealth,setBackupHealth]=useState<any>(null),[editingExpense,setEditingExpense]')
}

# Include settings modal in modalOpen body lock logic.
...
# Add openSettings function before setLockStatus if missing.
...
# Add Settings button next to Review in header if missing.
if ($content -notmatch 'settings-btn') {
  $content = $content.Replace('<button className="btn secondary review-btn" onClick={()=>setShowReview(true)}>Review</button><button className={`btn lock-btn ${isLocked?''secondary'':''}`} onClick={()=>setLockStatus(isLocked?''OPEN'':''LOCKED'')}>{isLocked?''Unlock month'':''Lock month''}</button>', '<button className="btn secondary review-btn" onClick={()=>setShowReview(true)}>Review</button><button className="btn secondary settings-btn" onClick={openSettings}>Settings</button><button className={`btn lock-btn ${isLocked?''secondary'':''}`} onClick={()=>setLockStatus(isLocked?''OPEN'':''LOCKED'')}>{isLocked?''Unlock month'':''Lock month''}</button>')
}

# Insert Settings modal before Review modal.
...
'@
  $content = $content.Replace('{showReview&&<div className="drawer-backdrop"', $settingsModal + '{showReview&&<div className="drawer-backdrop"')
}

Set-Content $pagePath $content -Encoding UTF8
Write-Host "Patch 25 page tweaks applied."
