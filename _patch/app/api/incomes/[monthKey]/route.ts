...
function calculateIncomeSummary(people: any[], monthlyIncomes: any[]) {
  ...
  for (const income of monthlyIncomes) if (income.isIncluded) totalsByPerson[income.personId] = (totalsByPerson[income.personId] ?? 0) + (income.amountHufMinor ?? 0);
...
export async function GET(_req: Request, ctx: { params: Promise<{ monthKey:string }> }) {
  ...
  const sources = await prisma.incomeSource.findMany({ where: { personId: { in: people.map(p => p.id) }, archivedAt: null, isOneTime: false }, orderBy: [{ personId: "asc" }, { sortOrder: "asc" }, { createdAt: "asc" }] });
  for (const source of sources.filter(s => s.isEnabled)) {
    await prisma.monthlyIncome.upsert({ where: { monthKey_incomeSourceId: { monthKey, incomeSourceId: source.id } }, update: {}, create: { monthKey, personId: source.personId, incomeSourceId: source.id, amountHufMinor: source.defaultAmountHufMinor, isIncluded: true } });
  }
