...
export async function GET(_req: Request, ctx: { params: Promise<{ monthKey:string }> }) {
  ...
  if (people.length < 2) return NextResponse.json({ settlement:null, error:"Nincs két aktív személy beállítva." }, { status:400 });
  ...
  const settlement = calculateSettlement({
    ...
    person1RatioBps: month.person1RatioBps ?? 5000,
    person2RatioBps: month.person2RatioBps ?? 5000,
    expenses,
  });
