...
export async function POST(req: Request) {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      const source = await tx.incomeSource.create({ data: { personId, name, defaultAmountHufMinor, isOneTime, isEnabled: isOneTime ? false : (body.isEnabled ?? true), archivedAt: isOneTime ? new Date() : null, sortOrder: count + 1 } });
      if (isOneTime && body.monthKey) {
        const monthKey = String(body.monthKey);
        await tx.month.upsert({ where: { monthKey }, update: {}, create: { monthKey } });
        await tx.monthlyIncome.create({ data: { monthKey, personId, incomeSourceId: source.id, amountHufMinor: defaultAmountHufMinor, isIncluded: true } });
      }
      return source;
    });
