...
export async function GET(req: Request) {
  ...
  const expenses = await prisma.expense.findMany({ where: { monthKey, deletedAt: null }, include: { paidByPerson:true, category:true, customSplits:true }, orderBy: [{ date:"desc" }, { createdAt:"desc" }] });
...
export async function POST(req: Request) {
  try {
    ...
    const expense = await prisma.expense.create({ data: { date, monthKey, description, currency, amountOriginalMinor, exchangeRateToHufMicros, amountHufMinor, paidByPersonId: body.paidByPersonId, splitType, categoryId: body.categoryId || null, note: body.note || null } });
    if (splitType === "CUSTOM_PERCENT") {
      ...
      await prisma.expenseCustomSplit.createMany({ data:[{ expenseId: expense.id, personId: people[0].id, ratioBps:p1Bps }, { expenseId: expense.id, personId: people[1].id, ratioBps:p2Bps }]});
    }
