...
export async function PATCH(req: Request, ctx: { params: Promise<{ id:string }> }) {
  try {
    ...
    if (existing.month?.status === "LOCKED") return NextResponse.json({ error: "This month is locked. Unlock it before editing incomes." }, { status: 423 });
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id:string }> }) {
  try {
    ...
    const existing = await getExistingMonthlyIncome(id);
    if (!existing) return NextResponse.json({ error: "Income row not found." }, { status: 404 });
    if (existing.month?.status === "LOCKED") return NextResponse.json({ error: "This month is locked. Unlock it before removing incomes." }, { status: 423 });
