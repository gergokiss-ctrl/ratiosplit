...
const splitLabels: Record<string,string> = { MONTHLY_RATIO:"Monthly ratio", EQUAL:"50/50", PERSON_1_ONLY:"Gergo only", PERSON_2_ONLY:"Partner only", CUSTOM_PERCENT:"Custom %", EXCLUDED:"Excluded" };
...
function money(minor:number, currency:string) { return new Intl.NumberFormat("en-US", { style:"currency", currency: currency === "OTHER" ? "HUF" : currency, maximumFractionDigits:2 }).format(minor/100); }
...
export default function Home() {
  ...
  async function createExpense(e: React.FormEvent) {
    ...
    try {
      ...
      setForm(f => ({...f, description:"", amount:"", note:""})); setMonth(data.monthKey); await loadMonth(); setMessage("Expense saved.");
  ...
  return <main className="shell">
    <header className="header">
      ...
      <div className="row" style={{maxWidth:540, justifySelf:"end"}}><div className="field"><label>Month</label><input value={month} onChange={e=>setMonth(e.target.value)} placeholder="2026-07" /></div><div className="field"><label>&nbsp;</label><a className="btn secondary" href={`/api/export/${month}`}>Export CSV</a></div></div>
    </header>
    ...
    <section className="layout">
      <div className="grid">
        <div className="card"><h2 className="panel-title">New expense</h2><form className="form" onSubmit={createExpense}>
          ...
          <button className="btn" disabled={loading}>{loading ? "Saving..." : "Save expense"}</button>
        </form></div>
        <div className="card"><h2 className="panel-title">Monthly ratio</h2><form className="form" onSubmit={saveRatio}>
          ...
          <button className="btn secondary" disabled={loading}>Save ratio</button>
        </form></div>
      </div>
      <div className="card"><div className="toolbar"><div><h2 className="panel-title">Expenses – {month}</h2><div className="muted">Excluded items: {huf(totals.excluded)}</div></div><button className="btn secondary" onClick={loadMonth}>Refresh</button></div>
        <div className="table-wrap"><table><thead><tr><th>Date</th><th>Description</th><th>Amount</th><th>HUF</th><th>Paid by</th><th>Split</th><th>Category</th><th></th></tr></thead><tbody>
          ...
          {!expenses.length && <tr><td colSpan={8} className="muted">No expenses recorded for this month yet.</td></tr>}
        </tbody></table></div>
